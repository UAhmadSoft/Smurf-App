...
const signToken = (id) => {
    return jwt.sign({id}, "abc-ooppqq-ok-secrete-key", {            // payload + secret + expire time
...
exports.login=catchAsync(async(req,res,next)=>{
  ...
  console.log(email)
  if (!email || !password) {                                                 //  check email and password exist
...
//   }
  ...
  console.log(user)
  if(!user) return next(new AppError(' please proveide user',401))
  if ( !user ||                                                  // check user exist and password correct
       !(await user.correctPassword(password,user.password))) {    // candinate password,correctpassword
